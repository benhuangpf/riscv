...
module TLFragmenter_8(
  ...
  wire [5:0]  _repeater_io_deq_bits_source;	// @[Fragmenter.scala:264:30]
  wire [31:0] _repeater_io_deq_bits_address;	// @[Fragmenter.scala:264:30]
  wire [3:0]  _repeater_io_deq_bits_mask;	// @[Fragmenter.scala:264:30]
  reg  [3:0]  acknum;	// @[Fragmenter.scala:191:29]
  reg  [2:0]  dOrig;	// @[Fragmenter.scala:192:24]
  reg         dToggle;	// @[Fragmenter.scala:193:30]
  wire        dFirst = acknum == 4'h0;	// @[Fragmenter.scala:191:29, :195:29]
  wire [4:0]  _dsizeOH1_T_1 = 5'h3 << auto_out_d_bits_size;	// @[package.scala:235:71]
  wire [5:0]  _GEN = {auto_out_d_bits_source[3:0], ~(_dsizeOH1_T_1[1:0])} & {1'h1, ~(auto_out_d_bits_source[3:0]), _dsizeOH1_T_1[1]};	// @[Cat.scala:33:92, Fragmenter.scala:194:41, OneHot.scala:64:12, package.scala:233:{35,47,49}, :235:{46,71,76}]
  ...
  wire        drop = ~(auto_out_d_bits_opcode[0]) & ~(auto_out_d_bits_source[5] ? dFirst : auto_out_d_bits_source[3:0] == 4'h0);	// @[Edges.scala:106:36, Fragmenter.scala:191:29, :194:41, :195:29, :196:30, :221:54, :224:{20,30,33,37}]
  wire        x1_d_ready = auto_in_d_ready | drop;	// @[Fragmenter.scala:224:30, :225:35]
  wire [2:0]  aFrag = _repeater_io_deq_bits_size > 3'h2 ? 3'h2 : _repeater_io_deq_bits_size;	// @[Fragmenter.scala:264:30, :287:{24,31}]
  wire [12:0] _aOrigOH1_T_1 = 13'h3F << _repeater_io_deq_bits_size;	// @[Fragmenter.scala:264:30, package.scala:235:71]
  wire [8:0]  _aFragOH1_T_1 = 9'h3 << aFrag;	// @[Fragmenter.scala:287:24, package.scala:235:71]
  reg  [3:0]  gennum;	// @[Fragmenter.scala:293:29]
  wire        aFirst = gennum == 4'h0;	// @[Fragmenter.scala:191:29, :293:29, :294:29]
  wire [3:0]  _old_gennum1_T_2 = gennum - 4'h1;	// @[Fragmenter.scala:293:29, :295:79]
  wire [3:0]  old_gennum1 = aFirst ? ~(_aOrigOH1_T_1[5:2]) : _old_gennum1_T_2;	// @[Fragmenter.scala:294:29, :295:{30,79}, package.scala:235:{46,71,76}]
  ...
  always @(posedge clock) begin
    if (reset) begin
      acknum <= 4'h0;	// @[Fragmenter.scala:191:29]
      dToggle <= 1'h0;	// @[Fragmenter.scala:193:30]
      gennum <= 4'h0;	// @[Fragmenter.scala:191:29, :293:29]
    end
    else begin
      if (_T_7) begin	// @[Decoupled.scala:51:35]
        if (dFirst)	// @[Fragmenter.scala:195:29]
          acknum <= auto_out_d_bits_source[3:0];	// @[Fragmenter.scala:191:29, :194:41]
        else	// @[Fragmenter.scala:195:29]
          acknum <= acknum - {3'h0, auto_out_d_bits_opcode[0] | auto_out_d_bits_size == 2'h2};	// @[Edges.scala:106:36, Fragmenter.scala:191:29, :206:{32,56}, :211:55, OneHot.scala:64:{12,27}]
      end
      if (_GEN_1)	// @[Fragmenter.scala:192:24, :210:27, :212:25, :213:19]
        dToggle <= auto_out_d_bits_source[4];	// @[Fragmenter.scala:193:30, :214:41]
      if (auto_out_a_ready & _repeater_io_deq_valid) begin	// @[Decoupled.scala:51:35, Fragmenter.scala:264:30]
        if (aFirst)	// @[Fragmenter.scala:294:29]
          gennum <= ~(_aOrigOH1_T_1[5:2]);	// @[Fragmenter.scala:293:29, package.scala:235:{46,71,76}]
  ...
  `ifndef SYNTHESIS
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        _RANDOM_0 = `RANDOM;
        acknum = _RANDOM_0[3:0];	// @[Fragmenter.scala:191:29]
        dOrig = _RANDOM_0[6:4];	// @[Fragmenter.scala:191:29, :192:24]
        dToggle = _RANDOM_0[7];	// @[Fragmenter.scala:191:29, :193:30]
        gennum = _RANDOM_0[11:8];	// @[Fragmenter.scala:191:29, :293:29]
        aToggle_r = _RANDOM_0[12];	// @[Fragmenter.scala:191:29, Reg.scala:19:16]
      `endif // RANDOMIZE_REG_INIT
    end // initial
  ...
  assign auto_in_d_bits_source = auto_out_d_bits_source[11:6];	// @[Fragmenter.scala:228:47]
  ...
  assign auto_out_a_bits_source = {_repeater_io_deq_bits_source, _repeater_io_deq_bits_opcode == 3'h0, ~(aFirst ? dToggle : aToggle_r), old_gennum1};	// @[Cat.scala:33:92, Fragmenter.scala:193:30, :264:30, :294:29, :295:30, :299:{23,27}, Mux.scala:81:61, OneHot.scala:64:12, Reg.scala:19:16]
  assign auto_out_a_bits_address = _repeater_io_deq_bits_address | {26'h0, ~(_aOrigOH1_T_1[5:0] | {old_gennum1, ~(_aFragOH1_T_1[1:0])} | 6'h3)};	// @[Fragmenter.scala:264:30, :295:30, :306:{49,51,100,111}, package.scala:235:{46,71,76}]
  assign auto_out_a_bits_mask = _repeater_io_full ? 4'hF : auto_in_a_bits_mask;	// @[Fragmenter.scala:264:30, :314:53, :315:31]
